...
export function DespesaModal({ despesa, onClose }: { despesa: Despesa | null, onClose: () => void }) {
  ...
  function handleDel() {
    if(!despesa || !confirm('Excluir?')) return
    startTransition(async () => {
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <select value={form.categoria} onChange={e=>setForm({...form, categoria: e.target.value})}
              className="h-11 w-full rounded-lg border border-white/[0.1] bg-[#101114] px-3 text-sm focus:border-[#d8f45a]/60">
              {['Filamentos', 'Insumos', 'Equipamento', 'Energia', 'Marketing', 'Software', 'Manutenção', 'Embalagens', 'Frete', 'Outros'].map(c=><option key={c}>{c}</option>)}
            </select>
          </div>
          ...
          <div className="mt-4 flex justify-between">
            {despesa ? <button type="button" onClick={handleDel} className="text-red-400 text-xs">Excluir</button> : <div/>}
